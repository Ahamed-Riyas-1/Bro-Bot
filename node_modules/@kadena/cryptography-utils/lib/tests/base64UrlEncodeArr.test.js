"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const vitest_1 = require("vitest");
const base64UrlEncodeArr_1 = require("../base64UrlEncodeArr");
(0, vitest_1.test)('Takes in Uint8Array binary object and outputs Base 64 URL encoded string', () => {
    const hshbin = new Uint8Array([
        205, 170, 167, 69, 13, 17, 99, 60, 83, 113, 200, 237, 98, 128, 111, 66, 192,
        232, 228, 175, 102, 198, 190, 19, 16, 95, 135, 33, 132, 226, 228, 154,
    ]);
    const actual = (0, base64UrlEncodeArr_1.base64UrlEncodeArr)(hshbin);
    const expected = 'zaqnRQ0RYzxTccjtYoBvQsDo5K9mxr4TEF-HIYTi5Jo';
    (0, vitest_1.expect)(expected).toEqual(actual);
});
//# sourceMappingURL=base64UrlEncodeArr.test.js.map